...
global FischUrl:="https://raw.githubusercontent.com/Steven55704/AHK/refs/heads/main/fisch.ahk"
global GrowAGardenUrl:="https://raw.githubusercontent.com/Steven55704/AHK/refs/heads/main/gag.ahk"
...
Gui Add, Text, x16 y70, % "Select Macro:"
Gui Add, DropDownList, x16 y90 w220 vSelectedMacro Choose1, Fisch|Grow A Garden
Gui Add, Checkbox, x16 y120 vRunMacro, % "Run macro after install"
...
progressmode() {
    global
    things := ["PathText","MainPath","Change","SelectedMacro","RunMacro","Continue","Cancel"]
    for _,v in things {
        GuiControl, Hide, % v
    }
...
Continue:
    ...
    GuiControlGet, selectedMacro,, SelectedMacro
    
    ; Set the download URL and directory based on selection
    if (selectedMacro = "Fisch") {
        global DownloadUrl := FischUrl
        subDir := "\Macro Fisch"
    } else if (selectedMacro = "Grow A Garden") {
        global DownloadUrl := GrowAGardenUrl
        subDir := "\Macro GAG"
    }
...
download(subDir) {
    ...
    DirPath:=DirPath subDir  ; Use the selected subdirectory
    MainPath:=DirPath "\main.ahk" 
    LibPath:=DirPath "\Lib"
    GdipPath:=LibPath "\Gdip_All.ahk"
    SkinDllPath:=LibPath "\SkinHu.dll"
    ConfigsPath:=DirPath "\Configs"  ; Changed from MGPath to ConfigsPath
    SkinsPath:=A_ScriptDir "\skins"  ; Changed to use A_ScriptDir
    
    ;Check folders
    FileCreateDir,%DirPath%
    FileCreateDir,%LibPath%
    FileCreateDir,%ConfigsPath%  ; Changed from MGPath
